...
class TermList {
	...
	override def toString() = if(first == null) "" else toString(first.toString, first.next);
	...
	def restore(backup: Array[Term]) : Unit = restore(first, backup)
	...
	def createLambdaVar(index: Int): LambdaVar = createLambdaVar(index, first)

	@tailrec private def createVar(id: String, node: Term) : Var = node match {
		case null => Var(id, this)
		case (v @ Var(id2, _)) if id == id2 => v ; // found it
		case u => createVar(id, u.next)
	}

	def createVar(id: String): Var = createVar(id, first)
	...
	@tailrec private def createFun(f: String, args: Array[Term], node: Term) : Fun = node match {
		...
		case (fun @ Fun(f2, args2, _)) if f == f2 &&
			...
			(0 until args.length).forall(i => args(i) eq args2(i)) => fun ; // found it
		case u => createFun(f, args, u.next)
	}

	def createFun(f: String, args: Array[Term]): Fun = if (args.isEmpty) {
		createFun(f, args, first)
	} else {
